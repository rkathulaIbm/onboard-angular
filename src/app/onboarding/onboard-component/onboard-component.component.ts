...
export class OnboardComponentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.onboardForm = this.formBuilder.group({
      ...
      planInCaseOfExtensionAmendmentRejection: ['', Validators.required],
    ...
    if (!this.isAddMode) {
      this.api.getAssociates().subscribe({
        next: (res: any) => {
          ...
          const results = res.find((obj: any) => {
            console.log(this.id, 'associate id >>>>', obj.associateId);
            return obj.associateId === parseInt(this.id);
          });
          console.log('filter data is>', results);
          this.onboardForm.patchValue(results);
        },
        error: () => {
          alert('Error');
        },
      ...
      //     .pipe(first())
      //     .subscribe(x => this.onboardForm.patchValue(x));
    }
    this.skillForm = [
  ...
  getSkillsData() {
    this.api.getSkillsDetails().subscribe({
      next: (res) => {
        this.skills = res;
      },
      error: () => {
  ...
  saveAssociateCompleteDetail() {
    ...
    if (this.onboardForm.valid && i == 0) {
      ...
      data.associate.endDate = data.associate.endDate
        .toISOString()
        .slice(0, 10);
      data.associate.asOnDate = data.associate.asOnDate
        .toISOString()
        .slice(0, 10);
      data.associate.clientExpDate = data.associate.clientExpDate
        .toISOString()
        .slice(0, 10);
      data.associate.itExpDate = data.associate.itExpDate
        .toISOString()
        .slice(0, 10);
      data.associate.ibmDate = data.associate.ibmDate
        .toISOString()
        .slice(0, 10);
      data.associate.visaMaxOutDate = data.associate.visaMaxOutDate
        .toISOString()
        .slice(0, 10);

      console.log('data', data);

      this.api.postAssociate(data).subscribe({
        next: (res) => {
  ...
  addUpdateAssociateDetail() {
    let dummy = {
      associate: {
        ...
        band: 'Test',
        primaryContact: 'Test',
        emailIbm: 'Test@ibm.com',
        emailClient: 'Test@ibm.com',
        xid: 'Xr123',
        clientManager: 'Test',
        endDate: '2022-05-14',
        location: 'Test',
    ...
    let data = {
      associate: this.onboardForm.value,
      associateSkill: [
        { skillId: 1, skillRating: '4' },
        { skillId: 2, skillRating: '3' },
      ],
    };
    console.log('dummy', dummy);
    // if(this.onboardForm.valid){
    this.api.postAssociate(dummy).subscribe({
      next: (res) => {
        alert('Associate added successfully');
      },
      error: () => {
